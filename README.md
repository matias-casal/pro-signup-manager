...
2.  **Setup Environment Variables:**
    The Docker setup ships with sensible defaults. Override any value by passing env vars (e.g., in a `.env` file or your shell) before running Compose.
...
> **Note:** The system automatically seeds the database with 10 demo professionals upon the first startup for immediate testing. Seeding runs from the backend container entrypoint; set `LOAD_DEMO_DATA=False` if you want to skip it or `DEMO_CSV_PATH` to point to another CSV.
